    ...
    public static class Utils
    {
        ...
        public static void MurderPlayer(PlayerControl killer, PlayerControl target, bool jumpToBody)
        {
            ...
            if (data != null && !data.IsDead)
            {
                ...
                if (killer.Is(RoleEnum.Advenger))
                {
                    var Advenger = Role.GetRole<Advenger>(killer);
                    if (target.Is(Faction.Impostors) ||
                        target.Is(RoleEnum.Glitch) && CustomGameOptions.AdvengerKillsGlitch ||
                        target.Is(RoleEnum.Arsonist) && CustomGameOptions.AdvengerKillsArsonist ||
                        target.Is(RoleEnum.Plaguebearer) && CustomGameOptions.AdvengerKillsPlaguebearer ||
                        target.Is(RoleEnum.Pestilence) && CustomGameOptions.AdvengerKillsPlaguebearer ||
                        target.Is(RoleEnum.Werewolf) && CustomGameOptions.AdvengerKillsWerewolf ||
                        target.Is(RoleEnum.Juggernaut) && CustomGameOptions.AdvengerKillsJuggernaut ||
                        target.Is(RoleEnum.Vampire) && CustomGameOptions.AdvengerKillsVampire ||
                        target.Is(RoleEnum.Executioner) && CustomGameOptions.AdvengerKillsExecutioner ||
                        target.Is(RoleEnum.Doomsayer) && CustomGameOptions.AdvengerKillsDoomsayer ||
                        target.Is(RoleEnum.Jester) && CustomGameOptions.AdvengerKillsJester) Advenger.CorrectKills += 1;
                    else if (killer == target) Advenger.IncorrectKills += 1;
                }
        ...
        public static void Convert(PlayerControl player)
        {
            ...
            if (player.Is(RoleEnum.Vigilante))
            {
                ...
                if (CustomGameOptions.SheriffCultistOn > 0) colorMapping.Add("Advenger", Colors.Advenger);
        ...
        public static void ResetCustomTimers()
        {
            ...
            if (PlayerControl.LocalPlayer.Is(RoleEnum.Advenger))
            {
                var Advenger = Role.GetRole<Advenger>(PlayerControl.LocalPlayer);
                Advenger.LastKilled = DateTime.UtcNow;
            }
            ...
            foreach (var vh in Role.GetRoles(RoleEnum.VampireHunter))
            {
                ...
                if (vamps.Count == 0 && vh.Player != StartImitate.ImitatingPlayer && !vh.Player.Data.IsDead && !vh.Player.Data.Disconnected)
                {
                    ...
                    if (CustomGameOptions.BecomeOnVampDeaths == BecomeEnum.Advenger)
                    {
                        Role.RoleDictionary.Remove(vhPlayer.PlayerId);
                        var kills = ((VampireHunter)vh).CorrectKills;
                        var Advenger = new Advenger(vhPlayer);
                        Advenger.CorrectKills = kills;
                        Advenger.RegenTask();
                    }
